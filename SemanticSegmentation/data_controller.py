...
class SegDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return rgb, depth, target  # 用于训练
        # return '{0}{1}'.format(self.root, self.path[index]), orig, rgb, target, depth  # 用于测试（无用）
        # return '{0}{1}'.format(self.root, self.path[index]), orig, orig_depth, rgb, target, depth  # 用于测试和可视化
