...
from segnet_withoutCM import SegNet as segnet
import sys
sys.path.append("..")
...
parser.add_argument('--n_epochs', default=70, help="epochs to train")
parser.add_argument('--workers', type=int, default=4, help='number of data loading workers')
parser.add_argument('--lr', default=0.0001, help="learning rate")
parser.add_argument('--logs_path', default='logs/', help="path to save logs")
parser.add_argument('--model_save_path', default='AblationStudy/trained_models_withoutCM_202307132321_1stStepTrain', help="path to save models")
parser.add_argument('--log_dir', default='logs/', help="path to save logs")
parser.add_argument('--resume_model', default='model_64_0.038625706425867976.pth', help="resume model name")
...
if __name__ == '__main__':
    ...
    # dataset = SegDataset(opt.dataset_root, '{0}/{1}'.format(opt.dataset_root, 'dataset_config/train_data_list_20230706.txt'), True, 15310)
    # dataloader = torch.utils.data.DataLoader(dataset, batch_size=opt.batch_size, shuffle=True, num_workers=int(opt.workers))
    # test_dataset = SegDataset(opt.dataset_root, '{0}/{1}'.format(opt.dataset_root, 'dataset_config/test_data_list_20230706.txt'), False, 3828)
    # test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=1, shuffle=True, num_workers=int(opt.workers))

    dataset = SegDataset(opt.dataset_root, '{0}/{1}'.format(opt.dataset_root, 'dataset_config/test_data_list_20230706.txt'), True, 2949)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=opt.batch_size, shuffle=True, num_workers=int(opt.workers))
    test_dataset = SegDataset(opt.dataset_root, '{0}/{1}'.format(opt.dataset_root, 'dataset_config/test_data_list_20230706.txt'), False, 2949)
    test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=1, shuffle=True, num_workers=int(opt.workers))
    ...
    if opt.resume_model != '':
        ...
        print('Successfully loaded resume weight!')
    ...
    for epoch in range(1, opt.n_epochs):
        ...
        if test_all_cost <= best_val_cost:
            ...
            torch.save(model.state_dict(), os.path.join(opt.model_save_path, 'best_model_{}_{}.pth'.format(epoch, test_all_cost)))
            print('----------->BEST SAVED<-----------')
        if epoch % 2 == 0:
